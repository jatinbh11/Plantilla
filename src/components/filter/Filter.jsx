...
function Filter() {
    ...
    const { mode, searchkey, setSearchkey, product, setFilteredProducts, filterType, setFilterType, filterPrice, setFilterPrice } = context;

    const handleCategoryChange = (e) => {
        const selectedCategory = e.target.value;
        setFilterType(selectedCategory);
        filterProducts(selectedCategory, filterPrice);
    };

    const handlePriceChange = (e) => {
        const selectedPrice = e.target.value;
        setFilterPrice(selectedPrice);
        filterProducts(filterType, selectedPrice);
    };
                ...
                <div className="p-5 rounded-lg bg-gray-100 drop-shadow-xl border border-gray-200"
                    ...
                    <div>
                        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mt-4">
                            <select value={filterType} onChange={handleCategoryChange} className="px-4 py-3 w-full rounded-md bg-gray-50 border-transparent outline-0 focus:border-gray-500 focus:bg-white focus:ring-0 text-sm" style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '', }}>
                                <option value="">All Categories</option>
                                {product.map((item, index) => (
                                    <option key={index} value={item.category}>{item.category}</option>
                                ))}
                            </select>
                            <select value={filterPrice} onChange={handlePriceChange} className="px-4 py-3 w-full rounded-md bg-gray-50 border-transparent outline-0  focus:border-gray-500 focus:bg-white focus:ring-0 text-sm" style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '', }}>
